...
function analysisSlide(pptx,c,state,analysisFor){const s=base(pptx,'تحلیل و اقدام');title(s,pptx,'تحلیل | '+c.title,c.id,'مبنای تحلیل: گزارش‌های ثبت شده، بدون ادعای علت یا نرخ خطر');const a=state.ai?.charts?.[c.id]||analysisFor(c),sections=[['۱','یافتهٔ قابل اتکا',a.observation],['۲','پرسش پیش از نتیجه‌گیری',a.question],['۳','اقدام برای بررسی',a.action]];
...
function closing(pptx,state,assets){const s=pptx.addSlide();s.background={color:P.navy};photo(s,assets['clinical-review'],0,0,4.1,H);s.addShape(pptx.ShapeType.rect,{x:4.1,y:0,w:.13,h:H,line:{color:P.teal},fill:{color:P.teal}});addIcon(s,pptx,'shield',11.85,.85,.69,'#FFFFFF');addText(s,'نتیجه‌گیری جامع گزارش',{x:4.7,y:1.45,w:7.85,h:.85},{fontSize:27,bold:true,color:P.white});const conclusion=state.ai?.conclusion||'۱. داده و پوشش هر نمودار را تأیید کنید.\n۲. موارد پرتکرار را با پرونده و حجم فعالیت بسنجید.\n۳. اقدام، مسئول و موعد بازبینی را در کمیته ثبت کنید.';addText(s,conclusion.slice(0,1250),{x:4.75,y:2.55,w:7.5,h:3.72},{fontSize:state.ai?.conclusion?13:16,color:'D9EDEB',valign:'top',breakLine:false});addText(s,'واحد ایمنی بیمار | '+state.period,{x:4.7,y:6.83,w:7.8,h:.27},{fontSize:10,color:'AFD6D3'});return s}
